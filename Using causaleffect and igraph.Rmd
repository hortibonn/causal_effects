...
Use the `igraph` model to get the formula for the causal effect of the nutrition intervention to plant trees `PT` on nutrition outcomes `N`. 

```{r get_nutrition_formula, message=FALSE, warning=FALSE}
ce1 <- causal.effect(y = "N", x = "PT", z = NULL, G = nutrition_decision, expr = TRUE)
```

Concatenate and print the formula output from the `causal.effect` output with `cat` from base R. 

```{r print_nutrition_formula, message=FALSE, warning=FALSE}
cat(ce1)
```
